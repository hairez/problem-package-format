---
layout: default
title: 2025-09 (latest)
sort: 2
---

# Problem package format

This is the `2025-09` version of the Kattis problem package format.
...
- To ensure fluid interoperation with external source control and archival tools, it is recommended that no file exceed 100 MiB in size.
...
---------------------- | --------- | --------------------------------------------- | -----------
`problem.yaml`         | Yes       | [Problem metadata](#problem-metadata)         | Metadata about the problem (such as source, license, and limits)
...
`submissions/`         | Yes       | [Example submissions](#example-submissions)   | Correct and incorrect judge solutions for the problem
`input_validators/`    | Yes       | [Input validators](#input-validators)         | Programs that verify correctness of the test data inputs
...
- For input validators: the files in the program directory of the input validator in question, and the contents of the `.files` directory of the test case being validated (if this directory exists). 
  It is an error for file names from `.files` and the input validator to collide.
...
Metadata about the problem (such as source, license, and limits) are provided in a YAML file named `problem.yaml` placed in the root directory of the package.
...
If using this version of the Format, it must be the string `2025-09`.
...
--------------- | --------------------------- | --------
`pass-fail`     | `scoring`                   | Default, and implied when the type does not include `scoring`. Submissions are judged as either accepted or rejected (though the "rejected" judgement is more fine-grained and divided into results such as "Wrong Answer", "Time Limit Exceeded", and other specific verdicts).
...
A person is specified as a string with the full name, optionally followed by an email wrapped in `<>`, (for example: `Full Name` or `Full Name <fullname@problem.example>`),
...
The `embargo_until` key, if present, declares that the problem package should not be made publicly available (in problem archives, online judges, or similar services) until a certain date and time.
...
(that is, the content that would be placed within a `document` environment).
...
`.out`                          | [Sample](#samples)                                  | Example team output, shown instead of `.ans`  
`.in.statement`                 | TODO                                                | Replaces the displayed `.in` file
`.ans.statement`                | TODO                                                | Replaces the displayed `.ans` file
`.interaction`                  | TODO                                                | Interaction shown for interactive problems
`.in.download`                  | TODO                                                | Replaces the downloadable `.in` file
`.ans.download`                 | TODO                                                | Replaces the downloadable `.ans` file
...
The output file `tc.out` must fail output validation with the given answer file `tc.ans`.
...
They consist of three files.
The input file `tc.in`, which must contain valid input.
The output file `tc.out` must pass output validation with the given answer file `tc.ans`.
...
For non interactive and non multi-pass problems the `.ans` files must pass the output validator if they are not overriden in any way (that is, if they are shown in the statement).
...
Submission programs (either a single file or a directory of files) **must** be placed in a direct subdirectory of `submissions`, for example `submissions/accepted/`.
...
excluding the trailing `.in`. Wildcards (`*`) only match within a file name (they do not match `/`).
...
Any submission that must satisfy a `required: TLE` requirement (that is, it must `TLE` on at least one test case) is used to provide an _upper bound_ on the time limit.
...
Note that this excludes submissions that for example have `required: [TLE, RTE]`.

Any submission that is not permitted to get `TLE` at all (on some subset of cases) (that is, it must satisfy a `permitted:` rule that does not contain `TLE`) is used to provide a _lower bound_ on the time limit.
...
If `case_sensitive` is specified, the two tokens must be identical byte for byte.
...
- If the output is correct for the input file (that is, the submission that produced the output is to be Accepted), the validator must exit with exit code 42.
...
The `require_pass` key may be used to indicate that `secret` or a test data group should only be scored if all test cases in `sample` or an earlier test data group received an accepted verdict; see [Group dependencies](#group-dependencies).
...
(Note that as a consequence, for the default `secret` configuration with a `max_score` of 100, every test data group of `secret` needs to specify a non-default `max_score`.)
